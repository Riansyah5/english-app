...
export default function LessonShow({ auth, lesson, isCompleted, nextLesson, personalNote }) {
    ...
    // Palet tema membaca yang selaras dengan mode global
    const themes = {
        light: {
            bg: 'bg-[#fafcfb]',
            ...
            noteBox: 'bg-slate-50/70 border-slate-200/80',
        ...
        sepia: {
            ...
            noteBox: 'bg-[#efe5d5] border-[#e2d0b6]',
            prose: 'prose-stone prose-headings:text-[#2c2216] prose-p:text-[#433422] prose-a:text-[#b45309] prose-strong:text-[#2c2216]'
        },
        dark: {
            bg: 'bg-[#0b1120]',
            card: 'bg-[#0f172a] border-slate-800 shadow-[0_4px_24px_-4px_rgba(0,0,0,0.5)]',
            text: 'text-white',
            textMuted: 'text-slate-400',
            inputBg: 'bg-[#0b1120] border-slate-700 text-white placeholder:text-slate-500',
            noteBox: 'bg-slate-800/60 border-slate-700/80',
    ...
    useEffect(() => {
        ...
        const savedTheme = localStorage.getItem('lessonTheme');
        
        if (savedTheme) {
            setTheme(savedTheme);
        } else {
            setTheme(isGlobalDark ? 'dark' : 'light');
        }
    }, []);
        ...
        <AuthenticatedLayout user={auth?.user}>
            ...
            <div className={`min-h-screen p-4 sm:p-6 md:p-8 transition-colors duration-300 font-sans ${currentTheme.bg}`}>
                <div className="max-w-4xl mx-auto space-y-7">
                    ...
                    {/* Top Navigation & Reading Ambient Switcher */}
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200/60 dark:border-slate-800">
                        <nav className="flex items-center gap-2 text-xs font-semibold">
                            <Link href="/lessons" className="text-slate-500 dark:text-slate-400 hover:text-[#ff822d] dark:hover:text-[#ff822d] transition-colors flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-2 self-start sm:self-center">
                            <div className="p-1 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-2xs flex items-center gap-1">
                                <button 
                                    onClick={() => changeTheme('light')} 
                                    className={`w-7 h-7 rounded-full flex items-center justify-center text-xs transition-all ${
                                        theme === 'light' ? 'bg-slate-900 text-white font-bold shadow-xs' : 'text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
                                    }`} 
                                    title="Mode Terang"
                                >
                                ...
                                <button 
                                    ...
                                    className={`w-7 h-7 rounded-full flex items-center justify-center text-xs transition-all ${
                                        theme === 'dark' ? 'bg-[#60f2ce] text-slate-950 font-bold shadow-xs' : 'text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
                                    }`} 
                                    title="Mode Gelap"
                                >
                            ...
                            <Link 
                                href="/lessons" 
                                className="px-3.5 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full shadow-2xs hover:bg-slate-50 dark:hover:bg-slate-700/60 transition"
                            >
                                Daftar Bab
                            </Link>
                        </div>
                    ...
                    <div className={`rounded-3xl border p-6 sm:p-10 transition-all ${currentTheme.card}`}>
                        ...
                        <div className="mb-8 pb-6 border-b border-slate-200/60 dark:border-slate-800">
                            <div className="flex flex-wrap items-center gap-2 mb-3">
                                <span className="px-3 py-1 rounded-xl text-xs font-mono font-bold uppercase bg-[#60f2ce]/20 dark:bg-[#60f2ce]/15 text-[#0d9488] dark:text-[#60f2ce] border border-[#60f2ce]/50 dark:border-[#60f2ce]/30">
                        ...
                        {/* Personal Notes Box */}
                        <div className={`p-5 sm:p-6 rounded-2xl border space-y-3.5 mb-8 shadow-2xs transition-colors ${currentTheme.noteBox}`}>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[11px] text-slate-400 dark:text-slate-400 m-0">Simpan rangkuman rumus atau kosakata penting dari bab ini</p>
                                    </div>
                                </div>
                            ...
                            <div className="flex justify-end">
                                <button 
                                    ...
                                    className={`inline-flex items-center gap-1.5 px-4 py-2 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold text-xs rounded-xl shadow-xs transition-all border border-transparent dark:border-slate-700 ${
                        ...
                        <div className="pt-6 border-t border-slate-200/60 dark:border-slate-800 text-center">
                            {completed ? (
                                <div className="space-y-4 animate-in fade-in duration-300">
                                    <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#60f2ce]/20 dark:bg-[#60f2ce]/15 text-[#0d9488] dark:text-[#60f2ce] border border-[#60f2ce]/50 dark:border-[#60f2ce]/30 text-xs sm:text-sm font-bold">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <p className="text-[11px] text-slate-400 dark:text-slate-500 block">
