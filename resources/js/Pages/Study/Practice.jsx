...
export default function StudyPractice({ auth, user, practiceCards = [], selectedType = '', source = 'today', limit = '50' }) {
    ...
    const navLinkClasses = (isActive) => 
        `px-4 py-2 text-xs font-bold transition-all rounded-xl ${
            isActive 
                ...
                <div className="max-w-3xl mx-auto space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex gap-2.5 items-center">
                            ...
                            <span className="px-3.5 py-1.5 bg-white border border-slate-200 text-[#0d9488] font-bold text-xs rounded-full shadow-xs">
                    ...
                    <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-[0_4px_24px_-4px_rgba(0,0,0,0.04)] space-y-4">
                        {/* Source Filter */}
                    ...
                    {isComplete ? (
                        ...
                        <div className="relative h-[440px] w-full mx-auto">
                            {/* Front Card */}
                            <div className={`absolute inset-0 w-full h-full bg-white rounded-3xl border border-slate-100 shadow-[0_12px_32px_-8px_rgba(0,0,0,0.06)] p-8 flex flex-col justify-between transition-all duration-300 transform ${
                                showAnswer ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'
                            }`}>
                                <div className="flex justify-between items-center">
                                    <span className="px-3 py-1 rounded-full text-[11px] font-bold bg-[#fcbf49]/15 text-[#b45309] border border-[#fcbf49]/30 uppercase tracking-wider">
                                ...
                                <div className="my-auto text-center px-4">
                                    <h2 className="font-extrabold text-3xl sm:text-4xl tracking-tight text-slate-900 leading-tight">
                                ...
                                <div>
                                    <button 
                                        ...
                                        className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-2xl shadow-sm transition-all"
                                    >
                                        Lihat Jawaban
                                    </button>
                                </div>
                            ...
                            {/* Back Card */}
                            <div className={`absolute inset-0 w-full h-full bg-white rounded-3xl border border-slate-100 shadow-[0_12px_32px_-8px_rgba(255,130,45,0.12)] p-8 flex flex-col justify-between transition-all duration-300 transform ${
                                !showAnswer ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'
                            }`}>
                                <div className="flex justify-between items-center">
                                    <span className="px-3 py-1 rounded-full text-[11px] font-bold bg-[#60f2ce]/20 text-[#0f766e] border border-[#60f2ce]/50 uppercase tracking-wider">
                                        Kunci Jawaban
                                    </span>
                                ...
                                <div className="my-3 space-y-4">
                                    <div className="text-center">
                                        <h3 className="font-black text-3xl sm:text-4xl tracking-tight text-[#ea580c]">
                                            {!isReversed ? studyItem?.translation : studyItem?.content}
                                        </h3>
                                    </div>
                                    
                                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                        <span className="text-[11px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Contoh Kalimat:</span>
                                ...
                                <div className="pt-2">
                                    <button 
                                        onClick={handleNext} 
                                        className="w-full py-3.5 bg-gradient-to-r from-[#ff822d] to-[#fcbf49] text-slate-950 hover:opacity-95 font-extrabold text-xs rounded-2xl shadow-md shadow-[#ff822d]/20 transition-all flex items-center justify-center gap-1.5"
                                    >
