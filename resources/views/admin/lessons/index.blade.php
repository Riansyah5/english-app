...
<div class="container py-4 position-relative z-1">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 gap-3">
        <div>
            <h2 class="fw-bold mb-2 text-white text-glow display-6" style="letter-spacing: -1px;">Kelola Isi Materi 📝</h2>
            <p class="text-slate mb-0 fw-medium" style="letter-spacing: 0.5px;">Manajemen bab pelajaran yang akan dibaca oleh pengguna.</p>
        </div>
        <a href="{{ route('admin.lessons.create') }}" class="btn btn-neon-primary fw-bold rounded-pill px-4 py-2 d-flex align-items-center gap-2">
            ...
            <div class="table-responsive">
                <table class="table-glass table-hover-glass align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-4 text-slate fw-medium">
                                <i class="bi bi-folder2-open me-1 opacity-75"></i> 
                            ...
                            <td class="fw-bold text-white fs-6" style="letter-spacing: 0.5px;">
                                {{ $lesson->title }}
                            </td>
                            <td>
                                @if($lesson->is_published)
                                    <span class="badge badge-neon-success px-3 py-2 fw-bold text-uppercase" style="letter-spacing: 1px; font-size: 0.7rem;">Terbit</span>
                                @else
                                    <span class="badge badge-neon-warning px-3 py-2 fw-bold text-uppercase" style="letter-spacing: 1px; font-size: 0.7rem;">Draft</span>
                                @endif
                            </td>
                            <td class="text-end pe-4">
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="{{ route('admin.lessons.edit', $lesson->id) }}" class="btn btn-sm btn-glass-info rounded-pill px-3 fw-semibold d-flex align-items-center gap-1">
                                    ...
                                    <form action="{{ route('admin.lessons.destroy', $lesson->id) }}" method="POST" class="d-inline">
                                        @csrf 
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-glass-danger rounded-pill px-3 fw-semibold d-flex align-items-center gap-1" onclick="return confirm('Yakin ingin menghapus bab materi ini secara permanen?')">
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-5">
                                <i class="bi bi-file-earmark-text display-4 text-slate opacity-25 d-block mb-3"></i>
                                <p class="text-slate fw-medium mb-0">Belum ada materi yang ditulis.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
