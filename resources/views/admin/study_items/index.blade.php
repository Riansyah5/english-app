...
<div class="container py-4">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 gap-3">
        <div>
            <h3 class="fw-bold mb-2 text-theme-main tracking-tight">Kelola Bank Materi 📚</h3>
            <p class="text-theme-muted mb-0 small">Manajemen kosakata, frasa, dan tata bahasa.</p>
        </div>
        <a href="{{ route('admin.study-items.create') }}" class="btn btn-minimal btn-minimal-primary btn-sm px-4 py-2 d-flex align-items-center gap-2">
            ...
            <table class="table-minimal table-hover-minimal align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge bg-minimal-badge border-minimal text-theme-main font-monospace text-uppercase px-2.5 py-1.5" style="font-size: 0.7rem;">
                                {{ str_replace('_', ' ', $item->type) }}
                            </span>
                        </td>
                        <td class="text-theme-muted small fw-medium">{{ $item->translation }}</td>
                        <td class="text-end pe-4">
                            <div class="d-flex justify-content-end gap-1.5">
                                <a href="{{ route('admin.study-items.edit', $item->id) }}" class="btn-action-flat btn-action-info" title="Edit Materi">
                                ...
                                <form action="{{ route('admin.study-items.destroy', $item->id) }}" method="POST" class="d-inline">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="submit" class="btn-action-flat btn-action-delete" title="Hapus Materi" onclick="return confirm('Hapus materi ini secara permanen?')">
                    ...
                    <tr>
                        <td colspan="4" class="text-center py-5 bg-minimal-badge">
                            <i class="bi bi-inbox display-4 text-theme-muted opacity-30 d-block mb-2"></i>
                            <span class="text-theme-muted small fw-medium">Belum ada materi di dalam database saat ini.</span>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<style>
/* ======================================================== */
/* MINIMALIST DESIGN SYSTEM - ADMIN STUDY ITEMS             */
/* ======================================================== */
